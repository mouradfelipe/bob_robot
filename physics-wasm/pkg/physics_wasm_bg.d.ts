/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export function __wbg_physicsworld_free(a: number): void;
export function physicsworld_new(): number;
export function physicsworld_get_part_position(a: number, b: number): number;
export function physicsworld_get_part_rotation(a: number, b: number): number;
export function physicsworld_begin_mouse_handle(a: number, b: number): void;
export function physicsworld_end_mouse_handle(a: number, b: number): void;
export function physicsworld_set_obstacle_position(a: number, b: number, c: number): void;
export function physicsworld_get_obstacle_position(a: number, b: number): number;
export function physicsworld_get_obstacle_rotation(a: number, b: number): number;
export function physicsworld_step(a: number): void;
export function physicsworld_set_timestep(a: number, b: number): void;
export function physicsworld_set_max_left_motor_torque(a: number, b: number): void;
export function physicsworld_set_max_right_motor_torque(a: number, b: number): void;
export function physicsworld_set_left_motor_target_speed(a: number, b: number): void;
export function physicsworld_set_right_motor_target_speed(a: number, b: number): void;
export function __wbindgen_malloc(a: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number): number;
export function __wbindgen_free(a: number, b: number): void;
export function __wbindgen_exn_store(a: number): void;
